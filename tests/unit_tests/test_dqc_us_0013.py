...
class TestDQC0013(unittest.TestCase):
    def setUp(self):
        ...
        mock_precondition_concept = mock.Mock(qname=m_qn_precondition, type=mock_type)
        mock_concept_no_blacklist = mock.Mock(qname=m_qn_no_blacklist, type=mock_type)
        mock_no_dimensions = {}
        ...
        self.fact_no_fire3 = mock.Mock(
            concept=mock_concept_no_blacklist, qname=m_qn_no_blacklist, value=-7777,
            namespaceURI='http://xbrl.sec.gov/us-gaap/2014-01-31',
            context=mock_context
        )
        ...
        self.fact_precondition_positive = mock.Mock(
            concept=mock_precondition_concept, qname=m_qn_precondition, value=77,
            namespaceURI='http://xbrl.sec.gov/us-gaap/2014-01-31',
            context=mock_context
        )
        # Precondition fact that will NOT cause the rule to fire (value is = 0)
        self.fact_precondition_zero = mock.Mock(
            concept=mock_precondition_concept, qname=m_qn_precondition, value=0,
            namespaceURI='http://xbrl.sec.gov/us-gaap/2014-01-31',
            context=mock_context
        )
        # Precondition fact that will NOT cause the rule to fire (value is < 0)
    ...
    def test_precondition_fact_exists(self):
        ...
        # Contains precondition element and the associated value is zero = No Fire
        fact_list2 = [self.fact_fire1, self.fact_fire2, self.fact_no_fire1,
                      self.fact_no_fire2, self.fact_no_fire3, self.fact_no_fire4,
                      self.fact_no_fire5, self.fact_precondition_zero]
        # Contains precondition element and the associated value is negative = No Fire
    ...
    def test_negative_number_with_dependence(self):
        ...
        blacklist_dict = util.neg_num.concept_map_from_csv(dqc_us_0013._DEFAULT_CONCEPTS_FILE)
        results1 = dqc_us_0013.filter_negative_number_with_dependence_facts(mock_fact_model1, blacklist_dict.keys())
        self.assertEqual(2, len(results1))
        results2 = dqc_us_0013.filter_negative_number_with_dependence_facts(mock_fact_model2, blacklist_dict.keys())
        self.assertEqual(0, len(results2))
        results3 = dqc_us_0013.filter_negative_number_with_dependence_facts(mock_fact_model3, blacklist_dict.keys())
        self.assertEqual(0, len(results3))
        results4 = dqc_us_0013.filter_negative_number_with_dependence_facts(mock_fact_model4, blacklist_dict.keys())
