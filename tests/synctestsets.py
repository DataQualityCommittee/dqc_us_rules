    ...
    for lineno, line in enumerate(lines, start=1):
        ...
        for i, entry in enumerate(infiles):
            if not isinstance(entry, dict):
                issues.append({"row": row_name, "line": lineno, "issue": f"INFILES[{i}] is not an object: {entry!r}"})
                continue

            missing_keys = {"file", "xule_run_only"} - set(entry.keys())
            extra_keys = set(entry.keys()) - {"file", "xule_run_only"}
            if missing_keys:
                issues.append({"row": row_name, "line": lineno, "issue": f"INFILES[{i}] missing key(s): {sorted(missing_keys)}"})
            if extra_keys:
                issues.append({"row": row_name, "line": lineno, "issue": f"INFILES[{i}] has unexpected key(s): {sorted(extra_keys)}"})
            if not entry.get("file", "").strip():
                issues.append({"row": row_name, "line": lineno, "issue": f"INFILES[{i}] has an empty 'file'"})
            if not entry.get("xule_run_only", "").strip():
                issues.append({"row": row_name, "line": lineno, "issue": f"INFILES[{i}] has an empty 'xule_run_only'"})
