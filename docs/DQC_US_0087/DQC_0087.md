...
To avoid false positives caused by historical lease facts that predate the current reporting period, the rule only evaluates facts whose period end date falls within 15 months of the current reporting period end date (i.e. `fact.period.end >= $ReportingPeriod - P1Y3M`). Facts with older period end dates are excluded.  

The rule applies to the following document types: 10-K, 10-KT, 20-F, 40-F, and their amended variants (10-K/A, 20-F/A, 40-F/A).  
...
## Appendix: Post-Implementation Changes

**March 2026:** Added reporting-period filtering to exclude lease facts with a period end date more than 15 months before the current reporting period. Expanded accepted document types to include amended form variants (10-K/A, 20-F/A, 40-F/A).
