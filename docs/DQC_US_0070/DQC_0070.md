...
The rule is designed to run only on those tables (XBRL cubes) that report the aggregate value of plan assets and does not attempt to aggregate the fact values on the level 3 reconciliation of plan assets. To achieve this the rule only adds the values of DefinedBenefitPlanFairValueOfPlanAssets that appear on a table with the following criteria:
...
If multiple tables are included in a filing, each table is checked as a separate iteration of the rule.
