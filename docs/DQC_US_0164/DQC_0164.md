...
## Rule function
This rule identifies where the filer has used the various location axis inappropriately.  The rule has four components.
...
### Component 4
This component checks if the descendants elements of  FairValueAssetsMeasuredOnRecurringBasisUnobservableInputReconciliationLineItems are used with the BalanceSheetLocationAxis, as fact values in the instance document.

### Component 5
This component checks if the descendants elements of  FairValueLiabilitiesMeasuredOnRecurringBasisUnobservableInputReconciliationLineItems are used with the BalanceSheetLocationAxis, as fact values in the instance document.  
...
A value of 24,164,000 is reported for the concept us-gaap:ServicingAssetAtFairValueAmount with the BalanceSheetLocationAxis. This concept should not be disaggregated using the BalanceSheetLocationAxis. Instead the FairValueByLiabilityAssetClassAxis should be used with the member  ABCMember.   

Rule Element ID: 10089  
Rule version: 22.0.0RC1

### Example rule message 5
A value of 10,000,000 is reported for the concept us-gaap:BusinessCombinationContingentConsiderationLiability with the BalanceSheetLocationAxis. This concept should not be disaggregated using the BalanceSheetLocationAxis. Instead the FairValueByLiabilityClassAxis should be used with the member  XYZMember.   
...
| DQC.US.0164.10089 | BalanceSheetLocationAxis |
| DQC.US.0164.10090 | IncomeStatementLocationAxis |
