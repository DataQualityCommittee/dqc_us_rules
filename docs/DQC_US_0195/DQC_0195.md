...
**Exception for treasury-stock gains (Rule 10627):** The check for retained earnings members (check 6, rule element 10627) is suppressed when a retained earnings increase is driven by a gain on treasury stock. Specifically, if `TreasuryStockCommonMember` exists in the filing and the retained earnings item is one of `StockIssuedDuringPeriodValueEmployeeBenefitPlan` or `StockIssuedDuringPeriodValueShareBasedCompensation`, the rule will not fire for that combination.
