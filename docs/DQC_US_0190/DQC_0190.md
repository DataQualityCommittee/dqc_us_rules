...
- **The sixth check (10605)** has been removed.
...
**Note (2024–2026 taxonomy):** The sixth check (10605) has been removed from the 2024, 2025, and 2026 rule files following a review of the FASB EBP implementation guidance. The LegalEntityAxis member validation logic has also been updated to use a taxonomy `navigate` approach to retrieve members (rather than covered-fact filtering), iterating over member local-names and skipping the `EntityDomain` member.
...
|--- |--- |
| DQC.US.0190.10600 |Any EBP element in a non-EBP filing|
