...
The two additional axis exclusions (RestructuringCostAndReserveAxis and srt:ProductOrServiceAxis) were added to prevent over-mapping in income-statement checks where these axes are used to break down amounts that do not require extensible enumeration treatment.

The rule also excludes facts whose period end date falls outside the current reporting window (i.e. facts where `fact.period.end < $ReportingPeriod - P1Y3M`) to avoid evaluating historical data when checking extensible enumeration compliance.
