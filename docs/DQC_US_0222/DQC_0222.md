...
- An additional upper bound of **$2.5 billion** was introduced for standard filers to capture likely scaling errors in filings where a very large public float is reported but revenue suggests the filer is smaller.  
- **Small-filer revenue exception:** If a filer's revenue is less than $100 million and the reported `EntityPublicFloat` is between $250 million and $700 million, the scaling-error flag is suppressed, as this may legitimately reflect a small company with a disproportionately large public float. The rule computes revenue using both standard and financial-services revenue concepts.
