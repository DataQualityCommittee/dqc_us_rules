...
To avoid false positives from historical lease facts that fall outside the current reporting window, the rule only evaluates facts whose period end date falls within 15 months of the current reporting period end date (i.e. `fact.period.end >= $ReportingPeriod - P1Y3M`). Older facts are excluded from evaluation.

The rule applies to the following document types: 10-K, 10-K/A, 10-KT, 20-F, 20-F/A, 40-F, and 40-F/A.
