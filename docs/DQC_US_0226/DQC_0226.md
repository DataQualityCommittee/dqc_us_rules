...
- The rule detects US GAAP elements by collecting facts covered by US-GAAP concepts into a fact list, deriving the top-10 concept names from those facts, and checking whether the list is non-empty. This fact-driven approach replaces a prior method that relied on taxonomy concept name intersection, to more reliably detect US-GAAP elements actually used in the filing rather than just defined in the taxonomy.
