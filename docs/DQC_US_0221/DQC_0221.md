...
- If any facts are reported using the `StatementBusinessSegmentsAxis` and neither `NumberOfReportableSegments` nor `NumberOfReportableSegmentsDisclosedByDefinitionFlag` is reported, the rule will trigger an error. The alternative concept `NumberOfReportableSegmentsDisclosedByDefinitionFlag` is an extension element that filers can use when the number of segments is disclosed by definition rather than explicitly stated; either concept satisfies the rule.
