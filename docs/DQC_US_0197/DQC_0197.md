...
Preferred stock items (`$Pref_stock_items`) are included in the set of income statement concepts before the allowed preferred-stock summation items (`$Pref_stock_items_allowed`) are subtracted. This ensures that the allowed preferred-stock items are correctly excluded from the final `$income_statement_items` set used in the check.
