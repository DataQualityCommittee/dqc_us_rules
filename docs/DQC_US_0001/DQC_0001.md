...
The extension member, Euro [Member] is used on the Currency [Axis] with the element Revenues.  Extension members should not be used with the Currency [Axis].  
...
The member, Land [Member] is used on the axis Position [Axis] with the element Property Plant and Equipment Gross.  This member should not be used with the Position [Axis].  
...
The extension member, {Rule.member.label}  is used on the axis {Rule.axis.label} with the element {fact1.label}.  Extension members should not be used with the {Rule.axis.label}.  
...
The member, {Rule.member.label} is used on the axis {Rule.axis.label} with the element {fact1.label}.  This member should not be used with the {Rule.axis.label].  
The properties of this ${fact1.name} fact are:  
Period: ${fact1.period}  
Dimensions: ${fact1.dimensions}  
Rule version: ${ruleVersion}
...
See DQC_0001_ListOfAxes spreadsheets for rule element ids.
